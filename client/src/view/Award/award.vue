<template>
    <div class="container" v-if="the_user && year_preview">
        <h3><center> Gama Awards {{year_show.ay_title}} Winners  </center></h3>
        ...
        <div class="row">
            ...
            <div class="col-lg-2">
                ...
                <select v-model="year_show" class="form-control">
                    <option v-for=" ay in year_preview " :value="ay" >
                        {{ay.ay_title}}
                    </option>
                </select>   <br>
            </div>
            <div class="col-lg-1"> <br> <center v-if="search!=''"> R : {{the_award_list.length}} </center></div>    
        </div>
        <div class="row">
...
export default {
    ...
    computed:{
        ...
        award_list_All(){
            ...
            return all_award.filter(item => { return item.al_year_id.indexOf(year_id.ay_id) > - 1 })
        },
        listFilter () {
